...
public class AssumptionsTest {
    ...
    @Test
    void testAccount() {
        assumingThat(!ledger.getAccountBalances().containsKey("sam"),
                () -> {
                    ledger.createAccount("sam");
                });
    }

    @Test
    void test() {
        assumingThat((Ledger.getInstance("bob", "test ledger 2", "harvard")) != null,
                () -> {
                    // cry
                    Ledger l = Ledger.getInstance("bob", "test ledger 2", "harvard");
                    l.createAccount("bob");
                });
    }
}
